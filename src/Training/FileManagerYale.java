...
public class FileManagerYale {
	...
	public static Matrix normalize(Matrix input){
		...
		Matrix result = new Matrix(243,320);
		for(int p = 0; p < 320; p ++){
			for(int q = 0; q < 243; q ++){
				double value = input.get(p*243+q, 0);
	...
	public static void convertMatricetoImage(Matrix x, int featureMode) throws IOException{
		...
		for(int i = 0; i < column; i ++){
			...
			BufferedImage img = new BufferedImage(320,243,BufferedImage.TYPE_BYTE_GRAY);
	...
	public static void convertToImage(Matrix input, int name) throws IOException{
		...
		for(int m = 0; m < 243; m ++ ){
			for(int n = 0; n < 320; n ++){
				int value = (int)input.get(n*243+m, 0);
				raster.setSample(n,m,0,value); 
			}
		}
